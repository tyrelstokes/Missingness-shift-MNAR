...
class PartialVAE:
    def __init__(self, pointnet, encoder, decoder, optimizer, config):
...
if __name__ == '__main__':
    ...
    ptnet = PointNet(x.shape[1], 64, 32, None, device, True, 'sum')
    enc = DenseGaussianNetwork([32, 16, 8]).to(device)
    dec = DenseGaussianNetwork([8, 16, 32, x.shape[1]]).to(device)
    opt = torch.optim.Adam(lr=1e-3, params=list(ptnet.parameters()) + list(enc.parameters()) + list(dec.parameters()),
                           weight_decay=1e-4)
    pvae = PartialVAE(
        ptnet, enc, dec, opt, {'batch_size': 128, 'max_epochs': 500, 'device': device}
    )

    pvae.fit(x)
    x_recon = pvae.reconstruct(x)

    x[m] = x_recon[m]

    out_file = os.path.basename(args.input_file).replace('.csv', '')
    out = pd.DataFrame(x, index=data.index, columns=data.columns)
    out.to_csv(os.path.join(args.save_dir, f'{out_file}_imputed.csv'))
